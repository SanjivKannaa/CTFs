...
NewYearCommonList = ["common", "common"]
NewYearHealthList = ["health", "health"]
NewYearFamilyList = ["family", "family"]
NewYearWealthList = ["wealth", "wealth"]
# random greeting
def random_greet(s):
...
def greeting():
	...
	try:
		...
		if fail not in greeting:
			bot_list = ["fail not in greeting case"]
			# bot_list = ["( ´ ∀ `)ノ～ ♡", "（✿◕ᴗ◕)つ━━✫・*。", "(๑˘ᵕ˘)"]
		else:
			bot_list = ["fail in greeting case"]
			# bot_list = ["(≖ ︿ ≖ ✿)", "ᕙ(⇀‸↼‶)ᕗ", "(≖ ︿ ≖ ✿)ꐦꐦ", "┌( ಠ_ಠ )┘"]
